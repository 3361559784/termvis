...
export async function runDoctor(argv, io) {
  ...
  const avatarDiagnostics = diagnoseAvatarRenderer({
    ...
    cwd: io.cwd,
    caps,
    symbolic: config.value?.life?.symbolic ?? true,
    chafaInfo: chafa
  });
  ...
  const readiness = {
    nonFallbackReady: Boolean(chafa.available && nodePty && caps.isTTY && !caps.termDumb && !caps.noColor && caps.colorDepth >= 8 && !avatarDiagnostics.fallback),
    requirements: {
      ...
      llm: llmResult.ok,
      imageProtocol: !avatarDiagnostics.fallback
    },
    fallbackReasons: []
  };
  ...
  if (!readiness.requirements.imageProtocol) readiness.fallbackReasons.push(`avatar fallback: ${avatarDiagnostics.reason}`);
  ...
  io.stdout.write(`  avatar:     ${avatarDiagnostics.mode} (${avatarDiagnostics.reason})\n`);
  if (avatarDiagnostics.suggestion) {
    io.stdout.write(`              -> ${avatarDiagnostics.suggestion}\n`);
  }
